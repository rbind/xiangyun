...
其中，$\hat{p_1} = a_1/n_1,\hat{p_2}=a_2/n_2$ 。检验统计量的极限分布是正态分布，这是一个 $u$ 检验[^1]。

[^1]: $u$ 检验，名称来自《概率论与数理统计教程》茆诗松版，有的书里也叫 $z$ 检验，总之，只要检验统计量的极限分布为标准正态分布。

## 手搓版本 {#handout}

这是一个双侧检验，极限分布是对称的，样本观察值落在拒绝域上、下两半部分的概率都要小于 $\alpha/2$ ，只有这样，拒绝原假设才有说服力。在显著性水平 $\alpha = 0.05$ 的情况下，对应的上、下临界值分别为 1.96 和 -1.96，图中黄色区域为拒绝域。

```{r}
# A/B 实验之假设检验 单样本 u 检验示意图
# 接受域
dat1 <- data.frame(x1 = seq(-1.96, 1.96, length.out = 100)) |>
  transform(y1 = dnorm(x1, mean = 0, sd = 1))
# 拒绝域
dat2 <- data.frame(x2 = c(seq(-4, -1.96, length.out = 100))) |>
  transform(y2 = dnorm(x2, mean = 0, sd = 1))
dat3 <- data.frame(x3 = seq(1.96, 4, length.out = 100)) |>
  transform(y3 = dnorm(x3, mean = 0, sd = 1))
# 临界值标记
dat4 <- data.frame(x4 = c(-1.96, 1.96)) |>
  transform(y4 = dnorm(x4, mean = 0, sd = 1))
# 拒绝域标记
dat5 <- data.frame(x5 = c(-1.96 - 0.5, 1.96 + 0.5), text = c("alpha/2", "alpha/2")) |>
  transform(y5 = dnorm(x5, mean = 0, sd = 1))

library(gglite)
g2() |>
  mark_area(data = dat1, encode = list(x = "x1", y = "y1", color = "#4C72B0"), style = list(fillOpacity = 0.5)) |>
  mark_area(data = dat2, encode = list(x = "x2", y = "y2", color = "#DD8452")) |>
  mark_area(data = dat3, encode = list(x = "x3", y = "y3", color = "#DD8452")) |>
  mark_line_x(data = dat4, encode = list(x = "x4", y = "y4", color = "#DD8452"), style = list(lineWidth = 3, lineDash = c(3, 3))) |>
  mark_text(data = dat4, encode = list(x = "x4", y = "y4", text = "x4", color = "#DD8452"), style = list(textAlign = "center", dy = -100, fontSize = 20)) |>
  axis_x(title = "x") |>
  mark_text(data = dat5, encode = list(x = "x5", y = "y5", text = "text", color = "#DD8452"), style = list(textAlign = "center", dy = -70, fontSize = 15, connector = TRUE, lineWidth = 1, startMarker = TRUE, startMarkerFill = "#4C72B0")) |>
  axis_y(title = "f(x)") |>
  title_(text = "u 检验示意图") |>
  theme_academy()
```
...
# 检验统计量的值
T_hat
# 显著性水平 alpha = 0.05 对应临界值 1.96
qnorm(p = 0.05 / 2)
qnorm(p = 1 - 0.05 / 2) # 关于均值 0 对称
# 双侧的落在拒绝域的概率 P 值
pnorm(q = abs(T_hat), lower.tail = F) + pnorm(q = -abs(T_hat), lower.tail = T)
# 简化等价于
2 * pnorm(q = T_hat) # 这是一个双侧检验，正态分布是对称的
```

检验结果显示，检验统计量的绝对值大于临界值 1.96，P 值小于 0.05，拒绝 H~0~ 。根据前面检验统计量的极限分布，检验统计量 $\hat{T}\sim N(0,1)$ ，那么 $\hat{T}^2 \sim \chi^2(1)$ 。换算成卡方分布，那么检验统计量和 P 值如下：
...
查看函数 `fisher.test()` 的帮助文档，可以知道 Fisher 精确检验的内容如下：

-   原假设 $H_0$：优势比为 1，
-   备择假设 $H_1$：优势比不为 1。

不难看出，两样本的二项分布的比例检验等价于检验优势比为 1 的 Fisher 精确检验。
...
其中，函数 `ES.h()` 用来计算两个比例的效应（Cohen's D）大小。对应的计算公式如下：

$$
h = 2 \arcsin \sqrt{p_1} - 2 \arcsin \sqrt{p_2}
$$

计算比例检验的功效，从 **pwr** 包的文档与源码中查看计算过程。
...
# Power 计算
pnorm(
  qnorm(sig.level / 2, lower = FALSE) - h * sqrt((n1 *
    n2) / (n1 + n2)),
  lower = FALSE
) +
  pnorm(
    qnorm(sig.level / 2, lower = TRUE) - h * sqrt((n1 * n2) / (n1 + n2)),
    lower = TRUE
  )
```
...
注意：功效计算函数 `power.prop.test()` 的检验统计量见前面卡方检验一节，使用到大样本下的正态近似。

```{r}
# 比率的绝对提升一样，但是 power 是不同的
power.prop.test(n = 46, sig.level = 0.05, p1 = 0.65, p2 = 0.45)
power.prop.test(n = 46, sig.level = 0.05, p1 = 0.25, p2 = 0.05)
```

在此，可以这么直观理解比率型指标提升的效应大小，提升一项指标刚开始总是容易些，越到后面，提升起来越难。

```{r collapse=TRUE}
library(pwr)
# arcsine transformation 来计算效应大小
ES.h(p1 = 0.65, p2 = 0.45)
ES.h(p1 = 0.25, p2 = 0.05)
pwr.2p.test(h = ES.h(p1 = 0.65, p2 = 0.45), sig.level = 0.05, n = 46)
pwr.2p.test(h = ES.h(p1 = 0.25, p2 = 0.05), sig.level = 0.05, n = 46)
```

样本量比较小时，**pwr** 包计算的结果与函数 `power.prop.test()` 不同。
