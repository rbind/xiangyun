...
其中，$\hat{p_1} = a_1/n_1,\hat{p_2}=a_2/n_2$ 。检验统计量的极限分布是正态分布，这是一个 $u$ 检验。

## 手搓版本 {#handout}

```{r echo=FALSE}
# A/B 实验之假设检验 单样本 T 检验
dat1 = data.frame(x1 = seq(-4, 4, length.out = 200)) |> 
  transform(y1 = dnorm(x1, mean = 0, sd = 1))

dat2 = data.frame(x2 = seq(-4, -1.96, length.out = 100)) |>
...
dat5 = data.frame(x5 = c(-1.96-0.5, 1.96+0.5), text = c("alpha/2", "1-alpha/2")) |> 
...
g2() |>
  ...
  mark_point(data = dat4, encode = list(x = "x4", y = "y4", color = "#4C72B0")) |>
  mark_text(data = dat4, encode = list(x = "x4", y = "y4", text = "x4", color = "#DD8452"), style = list(textAlign = "center", dy = -20, fontSize = 15)) |>
  axis_x(title = "x") |>
  mark_text(data = dat5, encode = list(x = "x5", y = "y5", text = "text", color = "#DD8452"), style = list(textAlign = "center", dy = -70,  fontSize = 20, connector = TRUE, startMarker = TRUE, startMarkerFill = "#4C72B0")) |> 
  axis_y(title = "f(x)") |>
  title_(text = "u 检验示意图") |>
  theme_academy()
```

在显著性水平 $\alpha = 0.05$ 的情况下，对应的上、下临界值分别为 1.96 和 -1.96，上图中黄色区域为拒绝域。下面计算检验统计量的值和 P 值。
