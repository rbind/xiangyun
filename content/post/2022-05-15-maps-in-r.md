...
<figure>
<img src="/img/maps-in-r/quakes-ggmap.png" class="full" alt="Figure 13: echarts4r 使用谷歌地图" /><figcaption aria-hidden="true">Figure 13: <strong>echarts4r</strong> 使用谷歌地图</figcaption>
</figure>
...
<figure>
<img src="/img/maps-in-r/quakes-mapbox.png" class="full" alt="Figure 14: echarts4r 使用 MAPBOX 地图" /><figcaption aria-hidden="true">Figure 14: <strong>echarts4r</strong> 使用 MAPBOX 地图</figcaption>
</figure>
...
<figure>
<img src="/img/maps-in-r/quakes-amap.png" class="full" alt="Figure 15: echarts4r 使用高德地图" /><figcaption aria-hidden="true">Figure 15: <strong>echarts4r</strong> 使用高德地图</figcaption>
</figure>
...
**sp** 会逐步退出空间数据领域，Roger Bivand 已经在为他的著作《Applied Spatial Data Analysis with R》迁移升级[代码](https://github.com/rsbivand/sf_asdar2ed)，推荐读者尽快转移到 **sf** 上。考虑到一些初学者肯定还会遇到大量 **sp** 为基础的空间数据处理、分析和可视化的案例代码，毕竟生态的迁移过程还要一段时间，因而做一些介绍。
...
本节主要采用 **leaflet** 包绘制交互地图，**leafletCN** 包内置一部分中国省、市边界地图数据，支持调用高德瓦片地图服务，提供部分合规的中国地图，核心函数是 `geojsonMap()`，它封装了 GeoJSON 格式的地图数据和绘图功能，更多定制参考`leaflet::leaflet()`。只要数据包含的一列区域名称和地图数据能映射上，就可以绘制出图<a href="#fig:china-leafletcn">40</a>，图中数据是随机生成的，调色板采用 `RdBu`，将连续的数据分段，每段对应一个色块。
...
<figure>
<img src="/img/maps-in-r/shaoyang-pop.png" class="full" alt="Figure 44: 邵阳市各个区县人口增长率" /><figcaption aria-hidden="true">Figure 44: 邵阳市各个区县人口增长率</figcaption>
</figure>
...
<figure>
<img src="/img/maps-in-r/china-hchinamap.png" class="full" alt="Figure 45: 中国地图数据" /><figcaption aria-hidden="true">Figure 45: 中国地图数据</figcaption>
</figure>
...
<figure>
<img src="/img/maps-in-r/china-linestring.png" class="full" alt="Figure 46: 静态地图数据" /><figcaption aria-hidden="true">Figure 46: 静态地图数据</figcaption>
</figure>
...
<figure>
<img src="/img/maps-in-r/china-province.png" class="full" alt="Figure 47: 静态地图数据" /><figcaption aria-hidden="true">Figure 47: 静态地图数据</figcaption>
</figure>

# 数据操作

先简要介绍一般数据操作，也是最常遇见的，然后介绍 **sf** 包和空间数据操作。
...
第三版的地图章节，如图 <a href="#fig:sf-nc">48</a>。在地图上绘制栅格图形，有个专业的制图术语 — [Choropleth Map](https://en.wikipedia.org/wiki/Choropleth_map)，翻译过来，大致叫填充地图、等值域图、瓦片图、围栏图、面量图、断层图等，可以非常直观地展示观测变量在地理区域内的变化情况。
...
<figure>
<img src="/img/maps-in-r/sf-nc.png" class="full" alt="Figure 48: sf 数据可视化" /><figcaption aria-hidden="true">Figure 48: sf 数据可视化</figcaption>
</figure>

## leaflet

而 **leaflet** 支持绘制交互式的图形，如图 <a href="#fig:leaflet-nc">49</a> 所示。稍微注意的是，需要先将数据映射到坐标参考系 EPSG:4326 上，而后 **leaflet** 会负责将数据转化映射到 EPSG:3857，详见[文档](https://rstudio.github.io/leaflet/projections.html)。
...
<figure>
<img src="/img/maps-in-r/leaflet-nc.png" class="full" alt="Figure 49: leaflet 数据可视化" /><figcaption aria-hidden="true">Figure 49: <strong>leaflet</strong> 数据可视化</figcaption>
</figure>
...
<figure>
<img src="/img/maps-in-r/latticeExtra-nc.png" class="full" alt="Figure 52: latticeExtra 数据可视化" /><figcaption aria-hidden="true">Figure 52: <strong>latticeExtra</strong> 数据可视化</figcaption>
</figure>
...
<figure>
<img src="/img/maps-in-r/lattice-volcano-shade.png" class="full" alt="Figure 53: Auckland Maunga Whau 火山三维地形图 \(10m\times 10m\)" /><figcaption aria-hidden="true">Figure 53: <a href="https://en.wikipedia.org/wiki/Maungawhau_/_Mount_Eden">Auckland Maunga Whau 火山</a>三维地形图 <code>\(10m\times 10m\)</code></figcaption>
</figure>

仅是图 <a href="#fig:lattice-volcano-shade">53</a> 就已经让 **ggplot2** 自愧不如了，更别说实现图<a href="#fig:lattice-volcano-contour">54</a>这样的组合图形，此例摘自 [Deepayan Sarkar](https://deepayan.github.io/) 的书《**lattice**: Multivariate Data Visualization with R》([Sarkar 2008](#ref-Sarkar2008))。

<figure>
<img src="/img/maps-in-r/lattice-volcano-contour.png" class="full" alt="Figure 54: Auckland Maunga Whau 火山带等值线的三维地形图 \(10m\times 10m\)" /><figcaption aria-hidden="true">Figure 54: Auckland Maunga Whau 火山带等值线的三维地形图 <code>\(10m\times 10m\)</code></figcaption>
</figure>
...
# 
# Package version:
#   baidumap_0.2.2      blogdown_1.7        echarts4r_0.4.3     ggmap_3.0.0        
#   ggplot2_3.3.5       knitr_1.37          lattice_0.20-45     layer_0.0.1        
#   leaflet_2.0.4.1     loa_0.2.47.1        plotly_4.10.0       raster_3.5.15      
#   RgoogleMaps_1.4.5.3 rmarkdown_2.11      sf_1.0-5            sp_1.4-6           
#   stars_0.5.5         terra_1.5.17       
# 
