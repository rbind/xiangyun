...
可以看出 **sp** 打造的一套空间数据基础类型及其衍生类型，知道数据类型才知道后续该怎么对其进行操作，调用合适的方法。比如最常使用的绘图函数 `plot()` 已经汇集很多方法了，它也是一个绘图对象，对于不同类型的数据对象，它会调用相应的绘图方法。在 R 语言内，一切都是对象，一切的操作都是函数调用。
    ...
    # Warning in matrix(c(methods(plot), ""), ncol = 4): data length [43]
    # is not a sub-multiple or multiple of the number of rows [11]

|                                            |                    |                  |                     |
|:-------------------------------------------|:-------------------|:-----------------|:--------------------|
| plot,ANY,ANY-method                        | plot.data.frame    | plot.HoltWinters | plot.shingle        |
| plot,Spatial,missing-method                | plot.decomposed.ts | plot.isoreg      | plot.spec           |
| plot,SpatialGrid,missing-method            | plot.default       | plot.lm          | plot.stepfun        |
| plot,SpatialGridDataFrame,missing-method   | plot.dendrogram    | plot.medpolish   | plot.stl            |
| plot,SpatialLines,missing-method           | plot.density       | plot.mlm         | plot.table          |
| plot,SpatialMultiPoints,missing-method     | plot.ecdf          | plot.ppr         | plot.trellis        |
| plot,SpatialPixels,missing-method          | plot.factor        | plot.prcomp      | plot.ts             |
| plot,SpatialPixelsDataFrame,missing-method | plot.formula       | plot.princomp    | plot.tskernel       |
| plot,SpatialPoints,missing-method          | plot.function      | plot.profile.nls | plot.TukeyHSD       |
| plot,SpatialPolygons,missing-method        | plot.hclust        | plot.R6          |                     |
| plot.acf                                   | plot.histogram     | plot.raster      | plot,ANY,ANY-method |
...
调 sp 数据对象特有的方法，获取图<a href="#fig:quakes-spplot">35</a>。
