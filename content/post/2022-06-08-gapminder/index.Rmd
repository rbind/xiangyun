...
笔者之前尝试过将人均 GDP / 预期寿命数据映射到地图上，制作地理可视化动画，如图\@ref(fig:wb-hans-life)所示，它补充了各个国家的空间位置信息，相比于之前的区域变量更加直观，可以展示任意时间点的空间分布，但只能展示一个数据指标。笔者建议在展示数据的时候，不妨同时提供多种展示形式，读者可任意选择指标、图形去探索、分析。
