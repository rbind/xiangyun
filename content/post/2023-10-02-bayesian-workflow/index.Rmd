...
yhat_gaussian <- fit_rongelap_gaussian$summary(variables = "yhat")
rongelap$pred_gaussian <- exp(yhat_gaussian$mean)
...
有了上面的背景知识（相关矩阵的分布，随机矩阵的分布），下面先在 R 环境中，模拟一组来自多元正态分布的样本。
...
数据概览，了解抽样结果，抽样的蒙特卡洛误差

```{r}
fit_multi_normal_gp$summary("y", .num_args = list(sigfig = 4, notation = "dec"))
```
