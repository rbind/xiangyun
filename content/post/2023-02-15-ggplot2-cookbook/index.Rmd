...
\@ref(fig:fig-gapminder-2007-income) 等价于 \@ref(fig:fig-gapminder-2007-income-stat)
...
\@ref(fig:fig-beta-plotmath) 中红线对应贝塔分布 $B(3,0.9)$ 而蓝线对应贝塔分布 $B(3,9)$
...
确认没有问题后，下面 \@ref(fig:fig-tikz-beta) 的坐标轴标签，标题，图例等位置都支持数学公式，也支持中文，使用 **tikzDevice** 打造出版级的效果图。更多功能的介绍见 <https://www.daqana.org/tikzDevice/>。
...
[**emojifont**](https://github.com/GuangchuangYu/emojifont) 包将 [Emoji 字体](https://github.com/ikatyang/emoji-cheat-sheet)引入 **ggplot2** 绘图，如 \@ref(fig:fig-emojifont) 所示。
...
[**patchwork**](https://github.com/thomasp85/patchwork) 包提供了一套非常简洁的语法，用来组合多个 ggplot2 图形。如 \@ref(fig:fig-patchwork) 所示，用散点图分别绘制 2002 年和 2007 年的数据，并将图形肩并肩的并排展示。
...
除了用竖线 `|` 左右并排，还可以用斜杠 `/` 做上下排列，见下 \@ref(fig:fig-patchwork-updown) 。
...
还可以引入括号 `()`，实现更加复杂的图形组合，见下 \@ref(fig:fig-patchwork-grid) 。
...
结合上面的介绍，不难看出，竖线 `|` 用于左右分割，斜杠 `/` 用于上下分割，而括号 `()` 用于范围的限定，下 \@ref(fig:fig-patchwork-layout) 是去掉括号后的效果。
...
[**geomtextpath**](https://github.com/AllanCameron/geomtextpath) 包新添文本路径图层，文本随路径变化，实现图文混合的效果，也省了图例，如 \@ref(fig:fig-text-path) 所示。
...
[**ggChernoff**](https://github.com/Selbosh/ggChernoff) 包受 Chernoff 表达多元数据的启发，借助从笑脸到苦脸的 Emoji 表情符号来表达数据指标。比如从低收入到高收入的连续型数据，从非常喜欢到非常不喜欢的有序分类型数据，用表情表达此类数据，显得非常形象。下 \@ref(fig:fig-chernoff) 以收入水平分类，在每个分类中，将预期寿命与表情结合，越长寿越开心。
