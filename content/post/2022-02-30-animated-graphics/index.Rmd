...
gapminder 数据集中 2002 年数据为例，绘图气泡图 \@ref(fig:gapminder-ggplot2)

![(\#fig:gapminder-ggplot2) ggplot2 制作静态气泡图](https://user-images.githubusercontent.com/12031874/145673108-e6c143a9-3010-4a5d-8a91-59c1e5c13b6c.png){.full}
...
如图\@ref(fig:gapminder-ggplot) 所示，各个洲的收入分布图

![(\#fig:gapminder-ggplot) ggplot2 制作累积分布图](/img/gapminder-ggplot.svg){.full}
...
![(\#fig:gapminder-echarts4r) echarts4r 制作网页动画 ](https://user-images.githubusercontent.com/12031874/145657748-0db6c4ee-47e9-4b1b-941d-1022937dcf4a.mov){.full}
...
下面采用 Carson Sievert 开发的 [plotly](https://github.com/plotly/plotly.R) 包[@Sievert2020]制作网页动画，仍是以 gapminder 数据集为例，示例修改自 plotly 官网的 [动画示例](https://plotly.com/r/animations/)。关于 plotly 以及绘制散点图的介绍，见前文[交互式网页图形与 R 语言](/2021/11/interactive-web-graphics/)，此处不再赘述[^fill-warn]。

[^fill-warn]: 读者看到 `plot_ly(fill = ~"",...)` 可能会奇怪，为什么参数 `fill` 的值是空字符串？这应该是 plotly 的 R 包或 JS 库的问题，不加会有很多警告：
...
![(\#fig:gapminder-plotly) plotly 制作网页动画](https://user-images.githubusercontent.com/12031874/145713395-5f692a24-e4b7-4900-94f2-bc0a4911ac28.mov){.full}

如图 \@ref(fig:gapminder-plotly) 所示，相比于 **echarts4r**， 气泡即使有重叠和覆盖，只要鼠标悬浮其上，就能显示被覆盖的 tooltip。

动画控制参数 `animation_opts()`，详见 Plotly [动画属性](https://github.com/plotly/plotly.js/blob/master/src/plots/animation_attributes.js)，结合此帮助文档，可知参数 `easing` 除了上面的取值 `linear`，还有很多，全部参数值见表\@ref(tab:easing)。
...
[^modebar]: 最后，顺便一说，添加如下 CSS 可以去掉图形右上角烦人的工具条。
...
::: rmdwarn
动画播放和暂停的控制按钮一直有问题，即点击播放后，按钮不会切换为暂停按钮，点击暂停后也不能恢复，详见 plotly 包的 Github [问题贴](https://github.com/plotly/plotly.R/issues/1207)。然而，Python 版的 plotly 模块制作此[动画](https://plotly.com/python/animations/)，一切正常，代码也紧凑很多，请读者接着往下看[^modebar]。
:::
...
![(\#fig:gapminder-plotly-python) Python 版 plotly 制作动画](https://user-images.githubusercontent.com/12031874/145701692-d2847ae6-6646-4fc7-bf9d-031a4e262555.mov){.full}
...
值得一提的是，支持丰富的调色板，涵盖常用的三大类型：无序的分类调色板、有序的分类调色板、连续型的调色板。调用其中一个调色板，只需 `px.colors.qualitative.Set2` 或者其逆序版本 `px.colors.qualitative.Set2_r`。也只需两行代码即可预览任意一组调色板， colorbrewer 调色板见图\@ref(fig:plotly-colorbrewer)。
...
![(\#fig:plotly-colorbrewer) plotly 的 colorbrewer 调色板](/img/plotly-colorbrewer.png){.full}
...
![(\#fig:gapminder-plotly-dark) plotly 的暗黑主题](https://user-images.githubusercontent.com/12031874/145711359-1fc543bd-d371-408b-abc0-929ae8fba4e4.mov){.full}
...
熟悉 **ggplot2** 绘图的读者，可能立马联想到这不和函数 `qplot()` 一样吗？是的，惊人地相似，请注意看下面绘图部分的代码和图\@ref(fig:gapminder-qplot)，也是将数据和几何元素、图层建立关系。
...
![(\#fig:gapminder-qplot) **ggplot2** 快速绘图函数`qplot()`](/img/gapminder-qplot.svg){.full}
...
[^x11-setup]: MacOS 从源码安装 R 软件时，配置需要指定 X11 相关库和头文件。
...
参考 SO 帖子[配置 X11](https://stackoverflow.com/questions/32207012)，先安装 [XQuartz](https://www.xquartz.org/)，再从源码安装 **rgl** 包，需要指定 X11 相关文件的路径，以便编译安装时发现[^x11-setup]。
