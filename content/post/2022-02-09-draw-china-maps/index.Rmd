...
本节主要采用 **leaflet** 包[@leaflet]绘制交互地图，**leaflet** 包调用的地图数据服务来自 OpenStreetMap 由 OpenStreetMap 基金会（OSMF）采用开放数据共享开放数据库许可协议（ODbL）授权，详见[协议文件内容](https://www.openstreetmap.org/copyright)。
...
[**leafletCN**](https://github.com/Lchiffon/leafletCN) 包函数 `leafletCN::amap()` 封装了高德瓦片地图服务的接口，**leaflet** 包函数 `leaflet::addTiles()` 默认提供了开放街道瓦片地图服务，只要符合瓦片服务的模版格式，就可以将 leaflet 服务替换成任意其他地图服务。
