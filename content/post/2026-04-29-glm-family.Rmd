...
> 在函数 `glm()` 中，伽马分布族是由均值 `mu` 和离散度 `disp` 两个参数来刻画的。对于伽马分布，当离散参数 `disp` 是 1 时，伽马分布的形状参数 `shape` 也是 1，这就退化为指数分布 $\mathrm{Exp}(\frac{1}{\sigma})$，伽马分布的尺度参数 `scale` 就是指数分布的期望 $\sigma$ 。
