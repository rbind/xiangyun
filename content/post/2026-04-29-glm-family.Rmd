...
其中，$\theta,\phi$ 是未知的参数，$\{a(\cdot),b(\cdot),c(\cdot)\}$ 是已知的函数，$x$ 是已知的协变量的观测值。$\theta$ 是依赖于 $x$ 的的自然参数（natural parameter，这个参数的形式仅取决于分布的形式），$\phi$ 是离散参数（dispersion parameter），更多介绍见文末参考文献。规范联系函数（canonical link function）由自然参数决定。
