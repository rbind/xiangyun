...
$$
f(y|x;\theta,\phi) = \exp\Big\{\frac{y\theta-b(\theta)}{a(\phi)}+c(y,\theta)\Big\}
$$

其中，$\theta,\phi$ 是未知的参数，$\{a(\cdot),b(\cdot),c(\cdot)\}$ 是已知的函数，$x$ 是已知的协变量的观测值。$\theta$ 是依赖于 $x$ 的的自然参数（natural parameter，这个参数的形式仅取决于分布的形式），$\phi$ 是离散参数 dispersion parameter，更多介绍见文末参考文献。

# 泊松分布 {#poisson}

响应变量服从泊松分布的广义线性模型（见下面的方程），在环境受到核辐射污染问题上，可用来刻画[辐射强度](/2023/05/nuclear-pollution/)（单位时间内辐射源释放的粒子数），在汽车保险的场景中，可以用来刻画[事故发生率](https://d.cosx.org/d/420881/2)（单位时间内事故发生的次数，一段时间内某投保群体发生事故的次数）。
...
## 离散参数与函数 `quasipoisson()` {#quasipoisson}
...
一个分布的离散程度是用该分布的方差与均值的比值来表示。泊松分布的离散参数 Dispersion parameter 设为 1，因为泊松分布的方差和均值是一样的。

值得注意，这里的离散参数和分布的变异系数 coefficient of variation 是不同的。变异系数是标准差（方差的平方根）与均值的比值（无量纲）。
...
回归系数是完全一样的，但是其相应的标准误差略有差别，离散参数的估计值是 0.9257 而不是 1 。分布族 `family` 参数值从 `poisson(link = "log")` 到 `quasipoisson(link = "log")` ，区别在于离散参数是当作固定值还是为待估参数。
...
## VGAM 和 glmnet 包 {#vgam-poissonff}
...
**glmnet** 包不仅包含函数 glm() 的功能，还支持变量选择（Lasso），处理共线性问题（岭回归）。

```{r message=FALSE}
library(glmnet)
# lambda = 0 去掉惩罚项
# 退化为一般的 GLM 
fit5 = glmnet(x = cbind(1, x), y = y, family = "poisson", lambda = 0)
# 回归系数
coef(fit5)
# Null deviance
fit5$nulldev
...
library(glmnet)
fit6 <- glmnet(
  x = cbind(1, x), y = y, offset = log(m),
  family = "poisson", lambda = 0
)
coef(fit6)
```

## 泊松分布还是二项分布？ {#car-insurance}

一个关于车险的带漂移项的真实数据案例见 **MASS** 包的数据 Insurance 的帮助文档（输入命令 `help(Insurance, package="MASS")`可查看帮助文档）。

数据集 Insurance 共有 5 个变量，其中， Holders 表示持有车险保单的数量，Claims 表示提出索赔的数量。
...
根据前面的基于模拟数据的模型介绍，下面基于数据集 Insurance 的代码如下。
...
fit7 = glm(Claims ~ District + Group + Age + offset(log(Holders)),
    data = Insurance, family = poisson(link = "log"))
summary(fit7)
```

根据问题的背景，可不可以理解为持有车险保单的人当中提出索赔的人的比例？从模型的角度理解为索赔率。因此，新的问题是响应变量假定服从带漂移项的泊松分布还是二项分布？

```{r}
# 响应变量是带漂移项的泊松分布与二项分布的区别
fit8 = glm(cbind(Claims, Holders-Claims) ~ District + Group + Age,
           data = Insurance, family = binomial(link = "logit"))
summary(fit8)
```

对数似然越大越好，AIC 越小越好，上述两个模型的比较。
...
无论从 AIC 还是对数似然来看，将响应变量视为服从二项分布更好。
...
**一个还没想明白的问题**：为什么是 logit 变换？上述方程中等号右手边 $X\bm{\beta}$ 原则上可以取到实数域上 $\mathbb{R}$ 任意值，而等号左手边中 $\bm{p}$ 取值范围是闭区间 $[0,1]$ 。从实数域 $\mathbb{R}$ 到 $[0,1]$ 区间的一一映射可以有许许多多，为什么是 logit 变换（以及相应的 Logistic 分布）？很多连续型随机变量的概率分布函数都是从实数域 $\mathbb{R}$ 映射到 $[0,1]$ 区间的。
...
如果不想让离散参数固定下来，可以用函数 `quasibinomial()` 替代 `binomial()` 。当遇到真实的数据时，数据一般是不会严丝合缝地服从二项分布的。样本均值与样本方差的关系不是精确的 $p$ 和 $p(1-p)$ 的关系。
...
伽马分布的概率密度函数带有两个参数 $\alpha$ 和 $\sigma$ ，其中，参数 $\alpha$ 称之为形状参数 shape parameter ，参数 $\sigma$ 称之为尺度参数 scale parameter 。伽马分布的均值和方差分别为 $\alpha\sigma$ 和 $\alpha\sigma^2$ 。$\Gamma(\cdot)$ 表示伽马函数（可查看函数 `gamma()` 的帮助文档），伽马分布的由来离不开伽马函数。

```{r eval=FALSE,echo=FALSE}
dat <- data.frame(
  id = seq(101),
  x1 = seq(0, 10, length.out = 101)) |>
  transform(y1 = dgamma(x1, shape = 1, scale = 1)) |> 
  transform(y2 = dgamma(x1, shape = 2, scale = 1)) |> 
  transform(y3 = dgamma(x1, shape = 2, scale = 2))

g2() |>
  mark_line(data = dat, encode = list(x = "x1", y = "y1", color = "#4C72B0"), style = list(lineWidth = 2, fillOpacity = 0.5)) |> 
  mark_line(data = dat, encode = list(x = "x1", y = "y2", color = "#DD8452"), style = list(lineWidth = 2, fillOpacity = 0.5)) |> 
  mark_line(data = dat, encode = list(x = "x1", y = "y3", color = "#66C2A5"), style = list(lineWidth = 2, fillOpacity = 0.5)) |> 
  axis_x(title = "x") |>
  axis_y(title = "f(x)") |> 
  theme_academy()
```
...
dat = transform(expand.grid(
  ...
  parameter == "shape = 1, scale = 1", dgamma(x, shape = 1, scale = 1),
  ifelse(parameter == "shape = 2, scale = 1", dgamma(x, shape = 2, scale = 1), dgamma(x, shape = 2, scale = 2))
))

g2() |>
  mark_line(data = dat, encode = list(x = "x", y = "y", color = "parameter"), style = list(lineWidth = 2, fillOpacity = 0.5)) |> 
  axis_x(title = "x") |>
  axis_y(title = "f(x)") |> 
  scale_color(palette = "Set2") |>
  legend_color(position = "right") |>
  titles(main = "Gamma 分布的概率密度函数图像") |> 
  theme_academy()
```

## 伽马分布可退化为指数分布 {#gamma-to-exp}

根据前述伽马分布的概率密度函数，当形状参数 $\alpha$ 等于 1 时，伽马分布退化为指数分布。若保持与前面指数分布 $\mathrm{Exp}(\lambda)$ 一致的记号，伽马分布退化下来的指数分布为 $\mathrm{Exp}(\frac{1}{\sigma})$ ，此时，期望为 $\sigma$ 。

$$
f(x;\sigma) = \frac{1}{\sigma}\mathrm{e}^{-\frac{x}{\sigma}}, \quad x \geq 0, \sigma > 0
$$

在 R 语言中，函数 `glm()` 的参数 `family` 支持选择伽马分布 `Gamma()`。

## 函数 `glm()` 中使用 Gamma 分布族 {#gamma-family}

注意函数 `Gamma()` 的[代码内容](https://github.com/r-devel/r-svn/blob/e9443159a3acbce40420058051bb1651825dab25/src/library/stats/R/family.R#L474)，以下这段话只属于这段代码。

> 在函数 `glm()` 中，伽马分布族是由均值参数 `mu` 和离散参数 `disp` 两个参数来刻画的。对于伽马分布，当离散参数 `disp` 是 1 时，伽马分布的形状参数 `shape` 也是 1，这就退化为指数分布 $\mathrm{Exp}(\frac{1}{\sigma})$，伽马分布的尺度参数 `scale` 就是指数分布的期望 $\sigma$ 。
...
1. Peng Ding, 2026, Linear Model and Extensions. Boca Raton, Florida. Chapman and Hall/CRC. <https://arxiv.org/abs/2401.00649>
