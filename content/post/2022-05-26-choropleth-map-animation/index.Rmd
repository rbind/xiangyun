...
1. **gganimate** 包[@gganimate]：提供一套动态图形语法，动画制作代码与 **ggplot2** 衔接得如丝般顺滑。
...
开发的 **tigris** 包封装了地图数据的下载接口，下面获取 2019 年发布的比例尺为 1:20,000,000 的州级地图数据，并将阿拉斯加州、夏威夷群岛和波多黎各适当移动至连续的 48 州下方。
...
在 sf 包的加持下，地理图层 `geom_sf()` 可以替代 `geom_map()`，坐标图层 `coord_sf()` 可以替代 `coord_map()`。
整个绘图的过程极大地简化下来了，核心部分只有两行，如下：
...
本节介绍两种动画形式 GIF 动画和 Web 动画，在上一节介绍的地区分布图基础上，让地图动起来，连续展示 共和党在 31 次美国大选中的得票率分布。
...
Thomas Lin Pedersen 从 David Robinson 接手维护**gganimate**后，大刀阔斧地开发，试图打造一套动态图形语法「A Grammar of Animated Graphics」，让 [**gganimate**](https://github.com/thomasp85/gganimate) 和 **ggplot2** 无缝衔接。与之前类似，首先对数据做些准备操作：
...
从 1856 年到 1976 年，美国共经历 31 次大选，设置总帧数 31，每秒播放 1 帧。

```{r}
options(gganimate.nframes = 31, gganimate.fps = 1)
```

[Gifski](https://gif.ski/) 是一个制作 GIF 动图的 Rust 库，[Jeroen Ooms](https://github.com/jeroen) 将其引入 R 语言社区，开发了同名的 R 语言接口 [**gifski**](https://github.com/r-rust/gifski) 包，**gganimate** 制作 GIF 动图或 MP4 格式视频都是调用 **gifski** 包做合成。

从 **ggplot2** 绘制的静态图形到 **gganimate** 制作 GIF 动图，只需一行代码，指定时间变量。
...
ggplot() +
  ...
  labs(
    title = "Voting Republican in {frame_time}",
    x = "", y = "", fill = ""
  ) +
  transition_time(year)
```
...
1.  调整总帧数和帧率，在图\@ref(fig:votes-repub-gganimate)基础上添加过渡效果，需要额外安装 **transformr** 包，它是通过中间差值的方式实现的。

1.  GIF 动图\@ref(fig:votes-repub-gganimate) 是逐年逐帧显示得票率分布，不需要添加过渡效果，美国大选每四年一届，插值后会很奇怪，但可以调整帧率，即每秒播放的地图分布图的幅数。

1.  **showtext** 包支持在 **ggplot2** 绘制的图形中 添加中文字体，但是不支持 **gganimate** 包的动画场景。
...
本小节了参考《Geocomputation with R》[@Lovelace2022]第 9 章第 3 节[动态地图](https://geocompr.robinlovelace.net/adv-map.html#animated-maps)内容
制作 GIF 动态地图。**tmap** 制作地图的代码风格和 **ggplot2** 的图形语法非常类似。
...
[**echarts4r**](https://github.com/JohnCoene/echarts4r)[@echarts4r] 是 [Apache ECharts](https://echarts.apache.org/en/) 的 R 语言接口，截止写作时间，R 包内置的 Apache ECharts JS 库更新到 5.2.2 版本。**echarts4r** 包目前已经支持了很多 Apache ECharts 的功能，一篇入门到应用文章参见[《echarts4r: 从入门到应用》](https://cosx.org/2021/12/introduction-to-echarts4r/)，本节的地图动画算是原文暂未涉及的一个应用例子。

制作交互式的地图动画之前，从 Github 下载 [**echarts4r.maps**](https://github.com/JohnCoene/echarts4r.maps) 包，它内置一份 GeoJSON 格式的美国州级地图数据。
...
- 悬浮提示：仍然采用默认的样式，读者可定制一番。稍注意的是在表达百分比时，JS 函数 `Math.round()` 会 四舍五入抹去小数点，要想保留两位小数，需要做一点变换 `Math.round(params.value * 100) / 100`。
...
- 坐标投影：相比于前面展示的动画，图\@ref(fig:votes-repub-echarts4r)采用的地图投影不一样，却是真实反应区域面积大小的。目前 **echarts4r** 还不支持其它投影，读者定制需要引入第三方 JS 库 [d3-geo](https://github.com/d3/d3-geo)。

- 数据映射：注意地图数据中州名 name 字段格式，比如阿拉斯加州是 `Alaska` 而不是 `alaska`，这个用于匹配观测数据中的 `state_name` 列。

- 动画播放：设置自动播放动画，播放间隔为 1s。

最终效果如图\@ref(fig:votes-repub-echarts4r)，笔者不了解美国的情况，仅从图\@ref(fig:votes-repub-echarts4r)不难看出，共和党在北方比较受欢迎。
...
1.  数据操作部分：
    ...
    -  数据结构的**变形**操作：`reshape()` 将原「宽格式」数据转化为「长格式」数据。
    -  部分数据的**筛选**操作：`subset()` 筛选 1976 年的美国大选数据。
...
总的来说，比较复杂的还是在于数据操作上，地图数据和观测数据的整洁、完整情况直接决定了制作的复杂度。
